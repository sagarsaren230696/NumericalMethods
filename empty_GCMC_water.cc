...
const double a_H2O = 2.665;
const double b_H2O = 0.5;
const double eps_H2O = 135.0;  // LJ epsilon, TraPPE, K
const double sig_H2O = a_H2O + b_H2O*(-0.672);  // LJ sigma, TraPPE, A
...
double GuestGuestEnergy(int which_ch4, std::vector<Particle> methanes) {
    ...
    for (int i = 0; i < methanes.size(); i++) {
        ...
        if (r2 <= r_c2) {
            ...
            double sigovrr6 = pow(sig_H2O * sig_H2O / r2, 3);
            E_gg += 4.0 * eps_H2O * sigovrr6 * (sigovrr6 - 1.0);
        }
    }
    return E_gg;
}
