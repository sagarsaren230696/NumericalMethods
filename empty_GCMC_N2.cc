...
const double eps_N2 = 36.0;  // LJ epsilon, TraPPE, K
const double sig_N2 = 3.310;  // LJ sigma, TraPPE, A
const double r_c = 14;  // A, LJ cutoff radius TraPPE
...
double GuestGuestEnergy(int which_ch4, std::vector<Particle> methanes) {
    ...
    for (int i = 0; i < methanes.size(); i++) {
        ...
        if (r2 <= r_c2) {
            ...
            double sigovrr6 = pow(sig_N2 * sig_N2 / r2, 3);
            E_gg += 4.0 * eps_N2 * sigovrr6 * (sigovrr6 - 1.0);
        }
    }
    return E_gg;
}
...
int main(int argc, char *argv[]) {
    ...
    std::vector<double> pressures(5); // (Pa)
    // 0.09999544991425668, 0.9995466582715875, 9.956322539258151, 19.832564485618395, 29.639419103720453
    pressures[0] = 0.0999 * 100000; 
    pressures[1] = 0.9995 * 100000; 
    pressures[2] = 9.9563 * 100000; 
    pressures[3] = 19.8325 * 100000;
    pressures[4] = 29.6394 * 100000;
    ...
    sprintf(outputname, "./results_empty_box_N2/SIM_U%.2f.txt", U_0);
    FILE *outfile;
    if (write_to_file) { 
